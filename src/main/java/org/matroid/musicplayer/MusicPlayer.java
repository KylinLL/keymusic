package org.matroid.musicplayer;
...
public class MusicPlayer extends Frame {
	...
	public MusicPlayer() {
		...
		labelfilepath = new Label("Directory：");
		labelfilename = new Label("File playing：");
	...
	private void open() {
		...
		if (filepath != null) {
			labelfilepath.setText("Directory：" + filepath);
	...
	private void play() {
		try {
			isStop = true;// 停止播放线程
			// 等待播放线程停止
			System.out.print("Start to play：" + filename);
			while (!hasStop) {
				System.out.print(".");
				try {
					Thread.sleep(10);
			...
			labelfilename.setText("File playing：" + filename);
